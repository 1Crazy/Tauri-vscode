...
const WORKBENCH_WEB_MAIN_CSS_PATH = path.join(APP_ROOT, 'out', 'vs', 'workbench', 'workbench.web.main.css');
const CODICON_TTF_SOURCE_PATH = path.join(APP_ROOT, 'node_modules', '@vscode', 'codicons', 'dist', 'codicon.ttf');
const CODICON_TTF_TARGET_PATH = path.join(APP_ROOT, 'out', 'vs', 'base', 'browser', 'ui', 'codicons', 'codicon', 'codicon.ttf');
...
function startServer(runnerArguments) {
	...
	const preloadModules = [
		'-r',
		path.join(APP_ROOT, 'scripts', 'code-web-require-hook.cjs')
	];

	console.log(`Starting @vscode/test-web: ${testWebLocation} ${runnerArguments.join(' ')}`);
	const proc = cp.spawn(process.execPath, [...preloadModules, testWebLocation, ...runnerArguments], { env, stdio: 'inherit' });
...
async function ensureCodeWebDevAssets() {
	await Promise.all([
		ensureWorkbenchWebMainCssPlaceholder(),
		ensureCodiconFont()
	]);
}

async function ensureWorkbenchWebMainCssPlaceholder() {
	if (await pathExists(WORKBENCH_WEB_MAIN_CSS_PATH)) {
		return;
	}

	await fs.promises.mkdir(path.dirname(WORKBENCH_WEB_MAIN_CSS_PATH), { recursive: true });
	await fs.promises.writeFile(
		WORKBENCH_WEB_MAIN_CSS_PATH,
		'/* Placeholder for @vscode/test-web in sources+esm mode. CSS modules load dynamically at runtime. */\n'
	);
}

async function ensureCodiconFont() {
	if (await pathExists(CODICON_TTF_TARGET_PATH)) {
		return;
	}

	if (!await pathExists(CODICON_TTF_SOURCE_PATH)) {
		fancyLog(`${ansiColors.yellow('code-web')}: Missing codicon font at ${CODICON_TTF_SOURCE_PATH}`);
		return;
	}

	await fs.promises.mkdir(path.dirname(CODICON_TTF_TARGET_PATH), { recursive: true });
	await fs.promises.copyFile(CODICON_TTF_SOURCE_PATH, CODICON_TTF_TARGET_PATH);
}
