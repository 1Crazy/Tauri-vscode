...
import { isTauriFileSystemBridgeAvailable } from '../../../../platform/files/browser/tauriFileSystemAccess.js';
...
export class FileDialogService extends AbstractFileDialogService implements IFileDialogService {
	...
	async pickFolderAndOpen(options: IPickAndOpenOptions): Promise<void> {
		...
		if (isTauriFileSystemBridgeAvailable(getActiveWindow())) {
			const result = await this.showOpenDialog({
				canSelectFiles: false,
				canSelectFolders: true,
				canSelectMany: false,
				defaultUri: options.defaultUri,
				availableFileSystems: this.addFileSchemaIfNeeded(schema, true)
			});

			const folderUri = result?.[0];
			if (folderUri) {
				return this.hostService.openWindow([{ folderUri }], { forceNewWindow: options.forceNewWindow, remoteAuthority: options.remoteAuthority });
			}
