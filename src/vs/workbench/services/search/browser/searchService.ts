...
import { getTauriFileSystemBridge } from '../../../../platform/files/browser/tauriFileSystemAccess.js';
import { revive } from '../../../../base/common/marshalling.js';
import { LocalFileSearchWorker } from './localFileSearch.js';
...
export class LocalFileSearchWorkerClient extends Disposable implements ISearchResultProvider {
	...
	private async cancelQuery(queryId: number) {
		this._worker?.proxy.$cancelQuery(queryId);
		this.mainThreadWorker.$cancelQuery(queryId);
	}
	...
	async textSearch(query: ITextQuery, onProgress?: (p: ISearchProgressItem) => void, token?: CancellationToken): Promise<ISearchComplete> {
		try {
			...
			await Promise.all(query.folderQueries.map(async fq => {
				...
				const searchWorker = this.shouldUseMainThreadSearch() ? this.mainThreadWorker : this._getOrCreateWorker().proxy;
				...
				const folderResults = await searchWorker.$searchDirectory(handle, query, fq, ignorePathCasing, queryId);
	...
	async fileSearch(query: IFileQuery, token?: CancellationToken): Promise<ISearchComplete> {
		try {
			...
			await Promise.all(query.folderQueries.map(async fq => {
				...
				const folderResults = await searchWorker.$listDirectory(handle, query, fq, caseSensitive, queryId);
				for (const folderResult of folderResults.results) {
	...
	private shouldUseMainThreadSearch(): boolean {
		return !!getTauriFileSystemBridge();
	}

	@memoize
	private get mainThreadWorker(): ILocalFileSearchWorker {
		const host: LocalFileSearchWorkerHost = {
			$sendTextSearchMatch: (match, queryId) => this.sendTextSearchMatch(match, queryId)
		};

		return new LocalFileSearchWorker(host);
	}
